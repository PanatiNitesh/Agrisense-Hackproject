...
def get_crop_recommendation_logic(farmer_doc: dict):
    """Core logic for crop recommendation. Returns a dictionary of results."""
    ...
    # Generate advice for existing and new crops
...
def get_yield_prediction_logic(farmer_doc: dict):
    """Core logic for yield prediction. Returns a dictionary of results."""
    yield_input = {
...
def get_cultivation_advice(client, crop_name, features):
    ...
    try:
        completion = client.chat.completions.create(model="openai/gpt-oss-120b", messages=[{"role": "user", "content": prompt}], temperature=0.7, max_tokens=512)
...
def get_intent_from_llm(client, query: str):
    """Uses the LLM to classify the user's intent from text."""
    ...
    try:
        completion = client.chat.completions.create(model="openai/gpt-oss-120b", messages=[{"role": "user", "content": prompt}], temperature=0.1, max_tokens=50)
...
def recommend_crop(req: FarmerRequest):
    """Endpoint for text-based crop recommendation requests."""
...
def predict_yield(req: FarmerRequest):
    """Endpoint for text-based yield prediction requests."""
    farmer_doc = coll.find_one({"farmerId": req.farmerId})
    if not farmer_doc:
        raise HTTPException(status_code=404, detail="Farmer not found")
    try:
...
def handle_voice_query(req: VoiceQueryRequest):
    """NEW: Endpoint to handle transcribed voice queries."""
    farmer_doc = coll.find_one({"farmerId": req.farmerId})
    if not farmer_doc:
        raise HTTPException(status_code=404, detail="Farmer not found")
