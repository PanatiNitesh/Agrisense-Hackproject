...
//     yieldComparison: [],
//   });
//   const [iotSensors, setIotSensors] = useState([]); // State for IoT sensors
...
//       });
//       setIotSensors(data.iotSensors || []); // Set IoT sensors from response
//     } catch (err) {
...
//   // NEW: Function to simulate sensor data update via API
...
//         await fetchDashboardData(); // Refresh dashboard to get new values
//       } else {
//         showNotification('Failed to update sensor data', 'error');
//       }
//     } catch (err) {
//       showNotification('Error simulating sensor data', 'error');
//     }
//   };
...
//   // Get soil pH from IoT sensors or fallback to farmerData
//   const getSoilPh = () => {
//     const phSensor = iotSensors.find(sensor => sensor.sensorType === 'ph' && sensor.status === 'active');
//     return phSensor ? phSensor.data.value : (farmerData?.ph || 'N/A');
//   };

//   // Get temperature from IoT sensors or fallback to farmerData
//   const getTemperature = () => {
//     const tempSensor = iotSensors.find(sensor => sensor.sensorType === 'temperature' && sensor.status === 'active');
//     return tempSensor ? tempSensor.data.value : (farmerData?.temperature || 'N/A');
//   };

//   // NEW: Get insights for soil pH
//   const getSoilPhInsight = (phValue) => {
//     if (typeof phValue !== 'number') return 'No data available for insights.';
//     if (phValue < 5.5) return 'Soil is acidic - Consider adding lime to raise pH.';
//     if (phValue > 7.5) return 'Soil is alkaline - Consider adding sulfur or organic matter.';
//     return 'Optimal pH level for most crops.';
//   };

//   // NEW: Get insights for temperature
//   const getTemperatureInsight = (tempValue) => {
//     if (typeof tempValue !== 'number') return 'No data available for insights.';
//     if (tempValue < 10) return 'Too cold - Risk of frost; protect crops.';
//     if (tempValue > 30) return 'High temperature - Increase irrigation to prevent stress.';
...
//     { subject: 'pH Level', A: ((getSoilPh() || 0) / 14) * 100, fullMark: 100 },
...
//   const StatCard = ({ title, value, unit, icon, trend, color = 'blue', insight }) => (
...
//       {insight && <p className="text-sm text-gray-500 mt-2 italic">{insight}</p>}
//     </div>
//   );
...
//           color="purple"
//           insight={getSoilPhInsight(getSoilPh())}
//         />
//         <StatCard 
//           title="Current Temp" 
//           value={`${getTemperature()}°C`} 
//           icon={<Thermometer className="w-6 h-6 text-white" />}
//           color="orange"
//           insight={getTemperatureInsight(getTemperature())}
//         />
//       </div>

//       {/* NEW: Button to simulate/update sensor data */}
//       <div className="flex justify-center">
//         <button
//           onClick={simulateSensorData}
//           className="flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-2xl hover:from-purple-700 hover:to-purple-800 transition-all duration-300 shadow-lg"
//         >
